...
class VSSM(nn.Module):
    ...
    # def forward(self, x):
    #     # Forward pass through the encoder and bottleneck layers
    #     x, x_downsample = self.forward_features(x)
        
    #     # Extract features before upsampling
    #     features = self.forward_up_features(x, x_downsample)
        
    #     # Upsample and perform final convolution for segmentation
    #     x_seg = self.up_x4(features)
        
    #     # Return both segmentation output and features
    #     return x_seg, features
...
class VSSM_classification(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, num_classes_cls=5, depths=[2, 2, 9, 2], 
                 dims=[96, 192, 384, 768], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, final_upsample="expand_first", **kwargs):
        super().__init__()
        ...
        # Define the index of the deepest encoder layer
        self.deepest_layer_index = self.num_layers - 1
        ...
        self.classification_head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(7*7*768, (7*7*768) // 2),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear((7*7*768) // 2, (7*7*768) // 4),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear((7*7*768) // 4, self.num_classes_cls)
        )

        # self.classification_head = nn.Sequential(
        #     nn.Linear(768 * 3, self.num_classes_cls)  # Adjust for the number of concatenated features
        # )
    ...
    def forward(self, x):
        ...
        # Extract features at the deepest layer
        deepest_features = x_downsample[self.deepest_layer_index]  # assuming you want the output before up layers

        # Use the deepest layer's features for classification
        class_logits = self.classification_head(deepest_features)

        # If you still need the full upsampling and segmentation
        features_up = self.forward_up_features(x, x_downsample)
        x_seg = self.up_x4(features_up)

        # Return both segmentation output and class logits
    ...
    #     # Select intermediate features to concatenate with deepest features
    #     selected_features = [
    #         x_downsample[0],  # First layer's output
    #         x_downsample[len(x_downsample) // 2],  # Middle layer's output
    #         x_downsample[-1]  # Deepest layer's output
    #     ]
        
    #     # Pooling each feature to 1x1 and then flattening
    #     pooled_features = [nn.AdaptiveAvgPool2d((1, 1))(feat).view(feat.size(0), -1) for feat in selected_features]
        
    #     # Debug: Print shapes of individual features to confirm size consistency
    #     for i, feat in enumerate(pooled_features):
    #         print(f"Feature {i} shape after pooling and flattening: {feat.shape}")
        
    #     concatenated_features = torch.cat(pooled_features, dim=1)
    #     print("Concatenated feature shape:", concatenated_features.shape)

    #     # Update classification head based on actual feature size
    #     self.classification_head = nn.Sequential(
    #         nn.Linear(concatenated_features.shape[1], self.num_classes_cls)  # Dynamically adjust based on concatenated feature size
    #     ).to("cuda")

    #     # Use the concatenated features for classification
    #     class_logits = self.classification_head(concatenated_features)

    #     # Continue with the segmentation path
    #     features_up = self.forward_up_features(x, x_downsample)
    #     x_seg = self.up_x4(features_up)
